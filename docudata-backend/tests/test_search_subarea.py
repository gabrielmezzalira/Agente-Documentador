...
def test_search_exige_subarea_valida(path, autenticar):
    from main import app

    response = autenticar(TestClient(app)).get(path)

    assert response.status_code == 422


def test_search_considera_somente_projetos_da_subarea(monkeypatch, autenticar):
    ...
    response = autenticar(TestClient(app)).get("/search?q=python&subarea=dev")
