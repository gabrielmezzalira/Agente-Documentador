...
def test_endpoints_de_settings_nao_expoem_segredo_e_substituem(monkeypatch, autenticar):
    ...
    assert client.get("/settings/gemini").status_code == 401
...
def test_put_rejeita_chave_vazia_ou_so_espacos(settings_store, api_key, autenticar):
    from main import app

    response = autenticar(TestClient(app)).put(
        "/settings/gemini/api-key",
...
def test_fluxo_generate_usa_mesma_chave_global_para_projetos_de_subareas_distintas(monkeypatch, autenticar):
    ...
    client = autenticar(TestClient(app))
...
def test_ausencia_da_chave_global_retorna_422_amigavel(monkeypatch, autenticar):
    import routers.generate as generate_router
    from main import app
    ...
    response = autenticar(TestClient(app)).post(
        "/generate",
...
def test_projeto_nao_tem_campos_gemini_e_endpoint_antigo_nao_existe(monkeypatch, autenticar):
